# -*- coding: utf-8 -*-
"""
Modified YOLO26 object detection benchmark for adenovirus in TEM images.

This script trains the modified YOLO26n/s/m/l/x object detection model variants
using a selected augmentation setup, evaluating each model on validation and test splits,
saving the best-performing weights, and exporting predictions and comparison
metrics.

@author: Olivier Rukundo, Ph.D., University of Limerick, July 26, 2026
olivier.rukundo@ul.ie or orukundo@gmail.com 
"""
...
_original_assigner_init = TaskAlignedAssigner.__init__

def adenovirus_assigner_init(self, *args, **kwargs):
    requested_topk = kwargs.get(
        "topk",
        args[0] if args else 13
    )

    if requested_topk > 1:
        kwargs["topk"] = 15

        if "topk2" in kwargs and kwargs["topk2"] is not None:
            kwargs["topk2"] = min(
                kwargs["topk2"],
                kwargs["topk"]
            )

    _original_assigner_init(
        self,
        *args,
        **kwargs
    )
    ...
    print(
        f"TaskAlignedAssigner active: "
        f"requested_topk={requested_topk}, "
        f"topk={self.topk}, "
        f"topk2={getattr(self, 'topk2', None)}, "
        f"classes={self.num_classes}"
    )

TaskAlignedAssigner.__init__ = adenovirus_assigner_init
...
def adenovirus_select_candidates_in_gts(
    self,
    xy_centers,
    gt_bboxes,
    mask_gt,
    ...
    wh = gt_bboxes_xywh[..., 2:]
    small_mask = wh < stal_threshold
    valid_small_mask = (small_mask * mask_gt).bool()

    expanded_wh = torch.full_like(
        wh,
        stal_expansion
    )
    ...
    gt_bboxes_xywh[..., 2:] = torch.where(
        valid_small_mask,
        torch.maximum(wh, expanded_wh),
        wh
    )

    expanded_bboxes = xywh2xyxy(gt_bboxes_xywh)

    lt, rb = expanded_bboxes.unsqueeze(2).chunk(
        2,
        dim=3
    )
...
TaskAlignedAssigner.select_candidates_in_gts = (
    adenovirus_select_candidates_in_gts
)
...
models_to_train = {
    ...
    "26m": {
        "architecture": "yolo26m-p2.yaml",
        "weights": "yolo26m.pt"
    },
    "26l": {
        "architecture": "yolo26l-p2.yaml",
        "weights": "yolo26l.pt"
    },
    "26x": {
        "architecture": "yolo26x-p2.yaml",
        "weights": "yolo26x.pt"
    }
}
...
def evaluate_model(
    ...
    return {
        "Model": model_name,
        "Weights": models_to_train[model_name]["weights"],
...
for model_name, model_config in models_to_train.items():
    ...
    print("\n" + "=" * 70)
    print(f"Training YOLO{model_name}-P2")
    print(f"Architecture: {architecture}")
    print(f"Transferred weights: {weights}")
    print("=" * 70)
    ...
    try:
        ...
        training_result = model.train(
            ...
            name=f"adenovirus_yolo{model_name}_p2_stal64x64_topk15",
            exist_ok=True,
            device=device,
            seed=random_seed,
            deterministic=True,
